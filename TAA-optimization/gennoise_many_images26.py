...
'''
from utils26 import get_adv_target, load_norm_mask, setup_attack_graph, report_extreme_values, load_many_images, get_print_triplets
import keras
...
def main(argv=None):
    ...
    for i in xrange(FLAGS.attack_epochs):
        ...
        if not FLAGS.fullres_input:
            _, noise_mul, train_loss, noisy_in, clean_loss, clean_classes, noisy_classes = sess.run( \
                (op, \
                varops['noise_mul'], \
                varops['adv_loss'], \
                varops['noise_inputs'], \
        ...
        print "Noise L2 loss is: %.5f"%(sess.run(tf.norm(noise_mul))),
        sys.stdout.flush()
        ...
        if i%FLAGS.save_frequency == 0 or proportion_misclassified > 0.1: 
            saver.save(sess, os.path.join('optimization_output', FLAGS.checkpoint, 'model', FLAGS.checkpoint), global_step=i)
            imsave(os.path.join('optimization_output', FLAGS.checkpoint, "noisy_images", "noisyimg_%s_epoch_%d.png"%(FLAGS.checkpoint,i)), (noisy_in[341]*255).astype(int))
        ...
        print 
       ### end of epoch
    imsave(os.path.join('optimization_output', FLAGS.checkpoint, "noise_mul", "noise_mul_Stop_SpeedLimit45.png"), (noise_mul*255).astype(int))
    np.save(os.path.join('optimization_output', FLAGS.checkpoint, "noise_mul", "noise_mul_Stop_SpeedLimit45.npy"), noise_mul)
    sess.close()
